#ifndef __STK_500_H__
#define __STK_500_H__
...
void stk500_drain();
int stk500_getsync();
int stk500_getparm(uint16_t parm, uint16_t* value);
int stk500_loadaddr(uint16_t addr);
int stk500_paged_write(AVRMEM* m, uint16_t n_bytes);
int stk500_program_enable();
void stk500_disable();
int stk500_read_sig_bytes(AVRMEM* m);

// *****************[ STK Message constants ]***************************

#define STK_SIGN_ON_MESSAGE "AVR STK"   // Sign on string for Cmnd_STK_GET_SIGN_ON
#define STK_PAGE_SIZE 128
#define STK_MAX_SYNC_ATTEMPTS 10
...
#define Parm_STK_TOPCARD_DETECT    0x98  // ' ' - Detect top-card attached
...
// **************************[Error messages]******************************//
#define ERROR_UNKNOWN_RESP 1 //stk500_disable(): unknown response=
#define ERROR_NO_DEVICE    2 
#define ERROR_PROTO_SYNC   3 //stk500_disable(): protocol error, expect=Resp_STK_INSYNC, resp=
#define ERROR_NO_SYNC      4  //can't get into sync
#define ERROR_NO_PROGMODE  5 //failed to get into programming mode
#define ERROR_NO_PGMR      6 //programmer not responding
#define ERROR_NOT_OK       7 
#define ERROR_PARM_FAILED  8 // get parameter failed: response
